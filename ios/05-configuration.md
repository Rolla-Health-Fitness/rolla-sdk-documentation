...
| `showOptionsButton` | `Bool` | No | `true` | Render a three-dot options action at the trailing edge of the Home top app bar. Tapping it opens an "Options" bottom sheet with shortcuts to Data Sources, Edit Goals, Leaderboards, and FAQ (rows follow your configuration — e.g. disabling the Leaderboards module hides its row). Defaults to true because most partners need this entry point |
