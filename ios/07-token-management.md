...
- `updateToken()` needs a running engine — any prior `show(from:)`, `openScreen`, `warmUpEngine()`, or headless call. If the engine has not started yet, initialize with the newest pair instead.
