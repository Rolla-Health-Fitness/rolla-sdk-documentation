...
- **[improvement] Hardened token handling.** The SDK ignores host-supplied tokens older than the pair it already holds, so a re-sent login pair can no longer break the session, and it holds a failing request for up to 10 seconds while `onTokenExpired` (Android) / `rollaDidRequestTokenRefresh` (iOS) is answered — a prompt `updateToken()` recovers with no visible error. See the [Android](android/06-token-management.md) / [iOS](ios/07-token-management.md) Token Management guides.
