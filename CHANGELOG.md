...
- **[improvement] Hardened token handling: stale tokens are ignored, and answering the expired-session callback recovers invisibly.** Re-presenting the SDK now re-sends the freshest token pair it has seen instead of the frozen `RollaConfiguration` values, and host-supplied tokens older than the pair the SDK already holds are ignored — a re-sent login pair can no longer break the session after the SDK rotated. When the SDK cannot refresh internally, it holds the failing request for up to 10 seconds while `onTokenExpired` (Android) / `rollaDidRequestTokenRefresh` (iOS) is being answered; an `updateToken()` push inside that window retries the request with no visible error. See the [Android](android/06-token-management.md) / [iOS](ios/07-token-management.md) Token Management guides.
