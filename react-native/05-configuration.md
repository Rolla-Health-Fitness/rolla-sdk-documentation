...
Everything you can shape about the SDK — branding, language, modules, data sources — is set through `RollaConfiguration`, the object you pass to every `Rolla` method. This page is the complete configuration reference: the full field table first, then a closer look at each configurable surface. Every value is passed to the native SDK verbatim; the wrapper never substitutes fallbacks of its own.

> **Configuration is read at engine start.** Apart from tokens, which you can push live with `updateToken()` (see [Token Management](06-token-management.md)), a `RollaConfiguration` applies for the engine's lifetime. To change branding, language, modules, or data sources, call `Rolla.destroyEngine()` and pass the new configuration to your next call — see [Engine Lifecycle](07-engine-lifecycle.md).
...
| `refreshToken` | `string` | No | unset | Refresh token for automatic credential renewal |
| `tokenExpiresIn` | `number` | No | unset | Token lifetime in seconds |
...
| `environment` | `'production' \| 'rnd'` | No | `'rnd'` | Target environment. See [Code Integration](04-code-integration.md#environment-values) |
| `language` | `RollaLanguage` | No | unset (profile-driven) | Forces the SDK UI language for the engine's lifetime. See [Language](#language) |
| `disabledModules` | `RollaDisabledModule[]` | No | `[]` (nothing disabled) | Modules whose entire UI is hidden across the SDK. See [Module Configuration](#module-configuration) |
| `disabledDataSources` | `RollaDataSource[]` | No | `[]` (all offered) | Data sources whose connect option is hidden wherever the user picks a source to connect. See [Data Source Configuration](#data-source-configuration) |
| `branding` | `RollaBranding` | No | unset | Visual identity overrides — every field optional, per-field merge. See [Custom Branding](#custom-branding-optional) |
| `showOptionsButton` | `boolean` | No | `true` | Render a three-dot options action at the trailing edge of the Home top app bar. Tapping it opens an "Options" bottom sheet with shortcuts into the SDK's features that you haven't disabled. Defaults to true because most partners need this entry point |
...
```ts
import type { RollaBranding, RollaConfiguration } from '@rolla-health/react-native-sdk';

const branding: RollaBranding = {
  hostAppName: 'Your App Name',              // names your app in SDK copy — see below
  primaryColor: '#1976D2',                   // seeds the SDK's entire color scheme
  themeMode: 'system',                       // 'system' | 'light' | 'dark'
  headerLogoAsset: undefined,                // partner logo asset path (provided by Rolla)
...
- **`primaryColor`** — seeds the whole SDK color scheme (buttons, navigation, inputs, charts, share cards) in both light and dark themes; it is not just an accent. A hex string, `'#RRGGBB'` or `'#RRGGBBAA'`, parsed natively on both platforms — do **not** use React Native's `processColor()`, which returns a platform-specific integer the SDK does not accept.
...
Image assets used by the SDK (such as partner logos referenced by `headerLogoAsset`) must be **pre-bundled inside the SDK** at build time — they cannot be shipped from your React Native bundle at runtime. The SDK runs in its own Flutter engine and has no access to your app's asset registry.
...
Rolla will bundle these into the SDK and provide the correct asset path to use in your `branding` configuration.
...
By default every module is enabled. To hide a module's entire UI everywhere it appears in the SDK, pass its `RollaDisabledModule` value in the `disabledModules` array (or omit the field / pass `[]` to keep everything enabled):
...
By default the SDK offers every data source the user can connect (Rolla Band, Garmin, Oura, Apple Health, Health Connect). To hide specific sources, pass their `RollaDataSource` values in the `disabledDataSources` array (or omit the field / pass `[]` to offer everything):
...
- **Deny-list semantics.** An empty array (the default) offers every source. Each value present hides that source. This matches `disabledModules`.
...
`Rolla.show()` and `Rolla.openScreen()` take an optional `{ transition }` that selects how the SDK UI animates in — the closing animation always mirrors the opening one. Omitting it keeps the default:

| Value | Animation |
|-------|-----------|
| `'default'` | The platform's standard presentation — a slide-in from the trailing edge on iOS, the standard activity animation on Android |
| `'fade'` | A cross-fade between your screen and the SDK UI, on open and close |

```ts
await Rolla.show(configuration, { transition: 'fade' });
```

## Invalid Values

Every union above mirrors a native SDK enum by its raw value, and the strings are passed to the native side verbatim. A value the SDK does not know — a misspelled module name, an unparsable color, an unknown language or transition — rejects the call with `INVALID_CONFIG` instead of being silently ignored, so a typo surfaces in development rather than as a missing feature in production. TypeScript catches most of these at compile time.
