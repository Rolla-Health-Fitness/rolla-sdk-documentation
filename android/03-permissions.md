...
The SDK's scheduled reminders are delivered by `flutter_local_notifications` through two broadcast receivers, `com.dexterous.flutterlocalnotifications.ScheduledNotificationReceiver` and `ScheduledNotificationBootReceiver`. The SDK's manifest declares both, so manifest merging adds them to your app — there is nothing for you to add. The one case that needs attention is an app that already declares these receivers because another Flutter module in it bundles `flutter_local_notifications` too: with the standard attributes (`android:exported="false"`) the merge is clean; different attributes need a `tools:replace`.
