...
| Symptom | Cause | Fix |
|---------|-------|-----|
| Works at first, but after ~30 minutes SDK screens show `Access denied (HTTP 401)`; exiting and re-entering the SDK fixes it — until it recurs | The SDK's refresh token was consumed outside the SDK (or never passed), and `onTokenExpired` goes unanswered. Re-entry only masks the problem: your app passes a fresh access token at initialization, which works for another 30 minutes | Responsibilities 2–4 |
